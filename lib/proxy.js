...
function proxyServer(option) {
  ...
  function save_client(client, msg, rcb) {
    if (!msg.proxy_path) {
      return rcb(new Error('must set "proxy_path" attr.'));
    }
...
function proxyClient(host, option) {
  ...
  var socket = sio.createWSClient(host || opt.server_url, opt);
	var repleaseCk = cookie_replease(option.server_url, opt.proxy_target);
...
//
// 代理如何正确的设置 cookie, 如果 cookie 的域是 tar, 将被替换为 src
// http://www.ibm.com/developerworks/cn/java/j-cookie/
//
function cookie_replease(src, tar) {
	var srcUrl = ulib.parse(src);
	var tarUrl = ulib.parse(tar);

	return function(resp) {
		var cookie = resp.headers['set-cookie'];
		if (cookie) {
			for (var i=0, e=cookie.length; i<e; ++i) {
				var cookieobj = ck.parse(cookie[i]);

				// ?? domain 有没有端口和路径 ??
				if (cookieobj.Domain == tarUrl.hostname) {
					cookieobj.Domain = srcUrl.hostname;
					var buf = [];
					for (var n in cookieobj) {
						buf.push(n);
						buf.push('=');
						buf.push(cookieobj[n]);
						buf.push('; ');
					}
					if (cookie[i].indexOf('HttpOnly')) {
						buf.push('HttpOnly');
					} else {
						buf.pop();
					}
